...
We can then identify some conditions for our group of interest (`x`) and our reference group (`y`). This step uses the kind of logic you would use in `dplyr::filter()` or `base::subset()`
```{r}
yoy <- # year over year
  demo_data() %>%
  add_date_columns(date) %>% 
  compare_conditions(
    ...
    cols = c(sales),   # the column(s) to aggregate
...
It might look funny to see `list(mean = mean)`. The name (left side) is how it will name the values, the right side is the function to use. If I had used `calc = list(avg = mean)` The names would have been `avg_x_*`. Because `compare_conditions()` uses the mean as the default, I'll omit it going forward. Now that I have my output as a list, I can use `headline_list()` to generate headlines. 
...
If your list has more than 2 values, you can specify the values you need by calling their names
```{r}
car_stats <-
  mtcars %>% 
  compare_conditions(
